...
> **版本：v4.9.3 | 更新：2026-08-15**
> **v4.0→v4.9 全演进一览**：`paper_output/research/CHANGELOG.md`（一眼看全 9 轮演进的定位/来源/新增/报告）
> **v4.9.1-v4.9.3 变更摘要（2026-08-14/15 门禁五波加固）：** 四轮对抗性审查/审计驱动的 fail-closed 化——审批门强制+行级消费、终检 verdict 两链共用 `blind_panel_schema` 校验、幽灵条目 CRITICAL、freshness 真接线、`_project_root.py` 锚定统一（commits 4de1292/615d99f/30f52b0/2b938af/546bbd7）。防御水位：空壳产物刷穿终检的旁路已封，穿透需伪造成磁盘证据；历史含第三方密钥，**filter-repo 清理决策前禁止 push**。逐波明细与审查报告位置见 `paper_output/research/CHANGELOG.md` v4.9.1-v4.9.3。
