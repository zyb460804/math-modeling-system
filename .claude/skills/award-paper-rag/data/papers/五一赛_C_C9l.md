...
边坡失稳是水利、交通和露天矿山工程中的典型渐进灾害。针对监测数据存在传感器零漂、安装偏差、缺失采样、短时异常和阶段转换不清等问题，本文建立“鲁棒传感器校正—持续变点识别—多源异常检测—分阶段融合预测—速度阈值预警”的一体化模型链条。针对问题一，以传统振弦式位移计为基准，构建Huber鲁棒线性校正模型，时间顺序交叉验证得到校正模型ŷ=0.8817509x−0.5299438，MAE为1.289 mm，RMSE为2.685 mm，MAPE为1.836%，R²为0.9992，五个待校正值为5.759、15.805、73.834、108.414、147.311 mm。针对问题二，采用Savitzky-Golay滤波、速度—加速度计算和分段残差下降准则识别三段式形变节点，得到T1=8025、T2=9575，三阶段平均速度分别为0.257、1.888、6.658 mm/h。针对问题三，按变量属性采用PCHIP插值、线性插值、非负约束、Savitzky-Golay滤波、滚动中位数和Hampel滤波完成补缺去噪，并用MAD标准化残差识别异常，训练集中检出单变量异常点71个、共同异常点16个；贡献分析表明深部位移、孔隙水压力、降雨量和微震事件数构成表面位移变化的主要解释链条。针对问题四，建立阶段趋势项与扰动残差修正项相结合的预测模型，实验集五个指定时刻表面位移预测值为5.538、54.806、390.035、566.748、576.693 mm。针对问题五，通过六选五变量组合枚举和时间序列交叉验证，选择降雨量、孔隙水压力、微震事件数、爆破点距离和单段最大药量作为最优组合；速度聚类得到0.764、4.126和8.0 mm/h三级阈值，形成正常、蓝色、橙色和红色预警规则。结果表明，本文模型具有较好的鲁棒性、可解释性和工程可操作性。
...
问题一的关键矛盾是新型光纤位移计相对基准传感器存在比例偏差和零点偏移，因此应构建监督校正模型。考虑现场监测数据可能存在局部异常点，采用Huber鲁棒回归替代普通最小二乘[7]，并用时间顺序交叉验证评价泛化误差。
...
使用Python中HuberRegressor求解，并采用时间顺序交叉验证评价泛化误差。全样本训练得到校正模型：
...
ϕᵢ(a)=∑ⱼ₌₋ₘᵐ(sᵢ₊ⱼ−∑ᵩ₌₀ᵖaᵩjᵩ)²，â=arg min ϕᵢ(a)，s̃ᵢ=â₀

题目采样间隔为10分钟，即每小时6个采样点。速度与加速度定义为：

vᵢ=6(s̃ᵢ−s̃ᵢ₋₁)，aᵢ=6(vᵢ−vᵢ₋₁)

候选节点 k 需满足前后窗口速度均值差异显著、节点后速度持续抬升，并能降低分段拟合残差。设未分段残差平方和为 RSS₀，分段后残差平方和为 RSS₁，则以 ΔRSS=(RSS₀−RSS₁)/RSS₀ 作为分段改进率。
...
需要说明的是，本文第 6 节给出的 T1 和 T2 属于离线阶段识别结果，主要用于准确刻画附件 2 的三阶段演化规律。在工程部署中，可将同一速度持续性准则嵌入滚动窗口，只利用当前时刻及其以前的监测数据实时更新候选变点，从而避免未来信息泄露，并形成可在线更新的阶段预警判据。
...
为避免正文表格过长，表 7.3 仅列出代表性共同异常点；按上述共同异常判据共识别共同异常点 16 个，完整清单见附录 C。实验集表面位移估计结果采用多源变量回归模型输出，节选结果见附录 D，完整结果由附录 A 程序自动生成。
...
本文共同异常检测采用“单变量残差异常同步叠加”的可解释策略，优点是规则清晰、便于定位异常来源；其局限是对“单变量均未越界但变量耦合关系异常”的情形刻画不足。实际推广时，可进一步引入马氏距离、Isolation Forest 或 Copula 联合异常检测，对降雨—孔压—深部位移—表面位移之间的联合响应异常进行识别，以提高临滑前兆捕捉能力。
...
预测值由 5.538 mm 逐步增至 576.693 mm，呈现缓慢增长、加速增长和快速增长三段特征。快速阶段误差相对较大，原因是该阶段位移增速高、扰动响应强，但 R² 仍保持在 0.9912；三个阶段残差均值均接近 0，表明模型不存在明显系统性高估或低估，能够兼顾趋势拟合和数值误差控制。

表 8.1 同时给出基于分阶段验证 RMSE 的近似 95% 预测区间，计算方法为点预测值±1.96×对应阶段 RMSE。该区间并非新的模型输出，而是将验证误差转化为风险量化尺度，用于判断高风险预测是否具有足够安全裕度。进一步地，工程部署中仍可采用 Bootstrap 残差重采样对预测区间进行滚动更新。
...
位移速度按 vᵢ=6(sᵢ−sᵢ₋₁) 计算，对正速度样本进行三类聚类，得到低速、中速和高速中心 c₁<c₂<c₃。考虑速度分布通常右偏，阈值采用相邻中心的几何均值。
...
从阈值位置看，0.764 mm/h 位于第一阶段平均速度 0.257 mm/h 与第二阶段平均速度 1.888 mm/h 之间，可用于识别由缓慢形变向加速形变过渡；4.126 mm/h 位于第二阶段平均速度 1.888 mm/h 与第三阶段平均速度 6.658 mm/h 之间，可用于识别加速形变向快速形变过渡；8.0 mm/h 高于快速阶段平均速度，作为临滑高风险判据。由此，阶段识别结果与预警阈值形成闭环联动。

上述速度阈值是基于当前边坡监测数据得到的场地自适应阈值，不宜直接作为所有边坡的固定常数。实际系统中应采用滚动窗口定期重估速度聚类中心和阈值，并结合孔隙水压力、降雨量和微震事件的同步异常对预警等级进行动态修正，从而提高不同监测阶段下的适应性。

同时，GB/T 32864—2016《滑坡防治工程勘查规范》强调滑坡防治工程需开展工程分级、勘查与稳定状态分析[13]。本文的多级速度阈值预警可作为接入规范化勘查、现场复核和稳定性分析流程的量化触发条件，从而增强预警规则的工程合规性。
...
问题四中，分阶段趋势项与扰动残差修正项相结合的模型给出实验集五个指定时刻预测值5.538、54.806、390.035、566.748和576.693 mm，预测曲线呈现缓慢增长、加速增长和快速增长的三段特征。问题五中，最优变量组合保留降雨量、孔隙水压力、微震事件数、爆破点距离和单段最大药量，速度聚类得到0.764、4.126和8.0 mm/h三条阈值，形成正常、蓝色、橙色和红色四类预警状态。
...
[13] 中华人民共和国国家质量监督检验检疫总局，中国国家标准化管理委员会. GB/T 32864-2016 滑坡防治工程勘查规范[S]. 北京：中国标准出版社，2016.
...
B1：问题一校正模型为ŷ=0.8817509x−0.5299438，时间顺序时间顺序五折交叉验证MAE=1.289 mm，RMSE=2.685 mm，MAPE=1.836%，R²=0.9992。
