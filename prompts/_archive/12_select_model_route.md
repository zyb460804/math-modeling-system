...
> **v4.5 公式核验前置（2026-08 新增，G4.9 门）**：若当前是历年真题（国赛/美赛/电工杯/五一赛等），**输出选模建议前必须先完成核心公式核验**：
> 1. 用官方参考答案/权威论文（award-paper-rag、citation-tracer、官方源检索）核对每个拟用公式；
> 2. 把核对结果写入 `paper_output/plan/formula_verification.md`（每条：公式名 / 论文用式 / 官方式 / 差异与误差量级 / 来源 / 结论：一致/近似可用/必须修正）；
> 3. **禁止凭空引入修正系数、经验系数或"根据实际测量确定"的公式参数**（实测教训：Q2 方向修正系数 k=0.3 无来源导致核心失分）；无法核验的公式标【待核验】，不写入正文。
> 非真题：在 formula_verification.md 中记录检索过程与理由，交人工确认。
